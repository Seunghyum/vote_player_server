...
export async function iterateCandidatesInPage(
  page: Page,
  browser: Browser,
  skip = 0
) {
  ...
  for (const el of elements.slice(skip)) {
    await el.click();
...
async function createCandidateInfoFromPage(page: Page, browser: Browser) {
  ...
  // const 의정활동menu = await $(
  //   page,
  //   "//*[@class='menu']//a[contains(text(),'의정활동')]"
  // );
  ...
  // if (!공동발의_의안menu)
  //   throw Error("공동발의 의안을 크롤링하지 못하였습니다.");
  // await 공동발의_의안menu.click();

  // await page.waitForSelector("tbody#prpl_cont__collabill__list tr", {
  //   timeout: 20 * 1000,
  // });
  // await sleep(2000); // NOTE 스크립트 로드 예방 차원
  // const collabills = await iterateAllBills({
  //   browser,
  //   page,
  //   is대표발의: false,
  // });
  ...
  console.log(
    chalk.red(`이름: ${koName}`)
    // chalk.yellow(`대표법안 길이: ${bills.length}`),
    // "/",
    // chalk.yellow(`공동법안 길이: ${collabills.length}`)
  );
